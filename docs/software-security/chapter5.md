...
C is still a popular if not the most popular programming language. Many compilers and languages are built using C. There are many vulnerabilities to watch for including character strings, buffer overflow, pointer handling, memory management, integer security, and formatted outputs

{: .important}
Remember that C is VERY flexible high level language. It is a weakly typed programming language that allows implicit type conversions. C also has relatively few restrictions on type mismatches, which can lead to unexpected behavior or security vulnerabilities. For example, C allows for pointer arithmetic, which can result in memory access errors or buffer overflows if not used carefully. C also allows for type punning, where a variable of one type is accessed as if it were a variable of another type, which can lead to undefined behavior or memory errors
...
## Type Safety
Type safety is defined as a feature of programming languages that helps prevent certain types of errors by enforcing type constraints. It ensures that operations are performed only on value that are compatible with the expected types. Type safety consists of two attributes
...
Type safety ensures that an operation on a particular type results in another value of that type, ensures predictable program behavior. Take for instance the following code
...
It is easy to make errors when copying and concatenating strings because many C lib functions do not bounds checking on copy operations
...
## Off-by-One Errors
An error occurring when a loop or other statement operates one time too many or too few, in other words the code accesses or modifies an element that is one position ahead or behind the intended range. This can happen with null-terminated strings
...
Eliminating the use of dangerous functions does not guarantee that your program is free from security flaws. Trying to avoid using functions like `strcpy()` or `strcat()`, and implementing your own functions can still be dangerous

# Buffer Overflow 
C/C++ are susceptible to buffer overflows. They both can define strings as null-terminated c-string, do not perform implicit bounds checking, and provide lib calls for strings that do not enforce bounds checking
...
{: .note}
A handy command in Linux is `readelf --sections ./a.out` to read the different sections of memory mapped
...
When manipulating the stack we can inject opcodes into the stack to be executed, the return address the we corrupt would put to the beginning of our exploit string, and the NOP instructions would slide us into the opcodes. Our buffer overflow can carefully be crafted with say `'\x90\x90\x90\x90' + opcodes + return address to the beginning of the NOP instructions`
...
Errors in string manipulation have long been recognized as the leading source of buffer overflows. Deploy a strategy that consistently secures string handling(prevention) and has back up methods that implement run-time detection and recovery schemes
...
1. To fight against code injection     
 a. Deploy stack canaries
2. Acquiring the EIP to point to attacker code       
  a. Make the stack non-executable
...
Stack Smashing Protector is a GCC extension for protecting applications written in C from the most common forms of stack buffer overflow exploits and is implemented as an intermediate language translator of GCC, it is typically enabled by default in most modern operating systems. Provides buffer overflow detection and variable reordering(local variables like buffers come after pointers like) to avoid corruption
